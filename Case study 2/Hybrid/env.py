...
class Flowsheet(Env):
    ...
    def step(self, action, sin):
        ...
        # ----------------------------------------- Column -----------------------------------------
            ...
            col = Column(
                f"DC{self.column_count}", nstages_c, distillation_rate, 2.5, press, sin
            )
        ...
        # ----------------------------------------- Column with recycle -----------------------------------------
            ...
            col = Column(
                f"DCR{self.column_count}", nstages_cr, mid_rate_cr, 2.5, press, sin
            )
            ...
            if self.sim.Convergence():
                ...
                f_cost = -0.48 * (1 + self.fixed_cost_column(Diam, Height))
                v_cost = (
                    -col.enery_consumption() * rr_cr / (10e3)
                )  # Variable cost (heat)
                cost = f_cost + v_cost  # Total cost

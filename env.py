...
class Flowsheet:
    ...
    def step(self, action, sin):
        ...
        # ----------------------------------------- Pump -----------------------------------------
            ...
            press_p = c_action
            pp = Pump(f"PP{self.pump_count}", press_p, sin)
        ...
        # ----------------------------------------- Adiabatic CSTR -----------------------------------------
            ...
            cstr_a = CSTR_A(f"AR{self.reac_count}", V, sin)

            sout = cstr_a.react()
            self.sim.EngineRun()

            if self.sim.Convergence():
                self.info[f"AR{self.reac_count}"] = [V, self.get_outputs(sout)]
        ...
        # ----------------------------------------- Adiabatic PFR -----------------------------------------
            ...
            D1 = c_action
            L1 = 2.185
            pfr = PFR_A(f"R{self.reac_count}", D1, L1, sin)
        ...
        # ----------------------------------------- Column -----------------------------------------
            ...
            press = c_action
            nstages_c = 12
            col = Column(f"DC{self.column_count}", nstages_c, press, sin)
        ...
        # ----------------------------------------- Column with recycle -----------------------------------------
            ...
            col = Column(f"DCR{self.column_count}", n_stages_c, press, sin)
            rr_cr = 0.9
            d, sout = col.distill()
            ...
            self.recpump_count += 1
            press_p = 2.4
            pp = Pump(f"REC{self.recpump_count}", press_p, rec)
            rec = pp.pump()
        ...
        #         f_cost = -0.48 * (1 + self.fixed_cost_column(Diam, Height))
        #         v_cost = (
        #             -col.enery_consumption() * rr_tcr / (10e3)
        #         )  # Variable cost (heat)
        #         cost = f_cost + v_cost  # Total cost
        ...
        if self.sim.Convergence():
            ...
            self.state = np.array(
                [
                    sout.get_temp(),
                    sout.get_press(),
                    sout.get_molar_flow("EO") / sout.get_total_molar_flow(),
                    sout.get_molar_flow("W") / sout.get_total_molar_flow(),
                    sout.get_molar_flow("EG") / sout.get_total_molar_flow(),
                    sout.get_molar_flow("DEG") / sout.get_total_molar_flow(),
    ...
    def reset(self):
        ...
        self.state = np.zeros((len(self.inlet_specs), 7))
